...
public class SwipeItem extends FrameLayout {
	...
	public SwipeItem(Context context, AttributeSet attrs, int defStyle) {
		...
		// 默认是右边缘检测
		int ordinal = a.getInt(R.styleable.SwipeItem_drag_edge,
				DragEdge.Right.ordinal());
		mDragEdge = DragEdge.values()[ordinal];
		// 默认模式是拉出
		ordinal = a.getInt(R.styleable.SwipeItem_show_mode,
				ShowMode.PullOut.ordinal());
		mShowMode = ShowMode.values()[ordinal];

		mHorizontalSwipeOffset = a.getDimension(
				R.styleable.SwipeItem_horizontalSwipeOffset, 0);
		mVerticalSwipeOffset = a.getDimension(
				R.styleable.SwipeItem_verticalSwipeOffset, 0);
	...
	private ViewDragHelper.Callback mDragHelperCallback = new ViewDragHelper.Callback() {
		...
		public void onViewReleased(View releasedChild, float xvel, float yvel) {
			...
			for (SwipeListener l : swipeListeners) {
				l.onHandRelease(SwipeItem.this, xvel, yvel);
			}
	...
	protected void dispatchSwipeEvent(int surfaceLeft, int surfaceTop,
		...
		if (!swipeListeners.isEmpty()) {
			...
			if (mEventCounter == 1) {
				if (open) {
					swipeListeners.get(0).onStartOpen(SwipeItem.this);
					swipeListeners.get(swipeListeners.size() - 1).onStartOpen(
							SwipeItem.this);
				} else {
					swipeListeners.get(0).onStartClose(SwipeItem.this);
			...
			for (SwipeListener l : swipeListeners) {
				l.onUpdate(SwipeItem.this, surfaceLeft - getPaddingLeft(),
						surfaceTop - getPaddingTop());
			}

			if (status == Status.Close) {
				swipeListeners.get(0).onClose(SwipeItem.this);
				swipeListeners.get(swipeListeners.size() - 1).onClose(
						SwipeItem.this);
				mEventCounter = 0;
			} else if (status == Status.Open) {
				getBottomView().setEnabled(true);
				swipeListeners.get(0).onOpen(SwipeItem.this);
	...
	private boolean isEnabledInAdapterView() {
		...
		if (adapterView != null) {
			...
			if (adapter != null) {
				int p = adapterView.getPositionForView(SwipeItem.this);
	...
	private void performAdapterViewItemClick(MotionEvent e) {
		...
		while (t != null) {
			if (t instanceof AdapterView) {
				...
				int p = view.getPositionForView(SwipeItem.this);
