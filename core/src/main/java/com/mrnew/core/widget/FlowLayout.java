...
public class FlowLayout extends ViewGroup {
    ...
    private final FlowLayoutConfiguration config;
    List<FlowLineDefinition> lines = new ArrayList<FlowLineDefinition>();

    public FlowLayout(Context context) {
        super(context);
        this.config = new FlowLayoutConfiguration(context, null);
    }

    public FlowLayout(Context context, AttributeSet attributeSet) {
        super(context, attributeSet);
        this.config = new FlowLayoutConfiguration(context, attributeSet);
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        lines.clear();
        FlowLineDefinition currentLine = new FlowLineDefinition(controlMaxLength, config);
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (newLine) {
                currentLine = new FlowLineDefinition(controlMaxLength, config);
        ...
        for (FlowLineDefinition l : lines) {
        ...
        for (FlowLineDefinition line : lines) {
    ...
    private void calculateLinesAndChildPosition(List<FlowLineDefinition> lines) {
        int prevLinesThickness = 0;
        final int linesCount = lines.size();
        for (final FlowLineDefinition line : lines) {
    ...
    private void applyPositionsToViews(FlowLineDefinition line) {
        final List<View> childViews = line.getViews();
        for (final View child : childViews) {
            LayoutParams layoutParams = (LayoutParams) child.getLayoutParams();
            if (this.config.getOrientation() == HORIZONTAL) {
    ...
    private void applyGravityToLines(List<FlowLineDefinition> lines, int realControlLength, int realControlThickness) {
        final int linesCount = lines.size();
        if (linesCount <= 0) {
            return;
        }
    ...
    private void applyGravityToLine(FlowLineDefinition line) {
